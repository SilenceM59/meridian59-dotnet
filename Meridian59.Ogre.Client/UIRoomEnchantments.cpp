...
namespace Meridian59 {
	...
	namespace Ogre
	{
		bool IsSafeRoom = false;
		bool IsPKPVPRoom = false;
		bool IsGuildPVPRoom = false;
		...
		void ControllerUI::RoomEnchantments::BuffAdd(int Index)
		{
				...
				imageComposersBuffs->Length > Index)
			{
				...
				if (buffObject->Name->Contains("Safe Room"))
				{
					// PVP is not allowed in this room.
					IsSafeRoom = true;
					IsGuildPVPRoom = false;
					IsPKPVPRoom = false;
				}
				if (buffObject->Name->Contains("Guild and Faction"))
				{
					// Murderer, Guild and Faction PVP only
					IsSafeRoom = false;
					IsGuildPVPRoom = true;
					IsPKPVPRoom = false;
				}
				if (buffObject->Name->Contains("Combat Allowed"))
				{
					// Open PVP (You can be PKed in this room)
					IsSafeRoom = false;
					IsPKPVPRoom = true;
					IsGuildPVPRoom = false;
				}
