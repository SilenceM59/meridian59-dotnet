...
namespace Meridian59 {
	namespace Ogre
	{
		void ControllerUI::StatusBar::Initialize()
		{
			...
			PasswordButton = static_cast<CEGUI::Window*>(Window->getChild(UI_NAME_STATUSBAR_PASSWORDBUTTON));
			...
			// password button (change password window)
			PasswordButton->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(UICallbacks::StatusBar::OnPasswordButtonClicked));
		...
		void ControllerUI::StatusBar::ApplyLanguage()
		{
			...
			PasswordButton->setTooltipText(GetLangTooltipStatusBar(LANGSTR_TOOLTIP_STATUSBAR::PASSWORD_TOOLTIP));
		...
		bool UICallbacks::StatusBar::OnPasswordButtonClicked(const CEGUI::EventArgs& e)
		{
