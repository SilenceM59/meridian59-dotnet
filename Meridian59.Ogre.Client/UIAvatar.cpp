...
namespace Meridian59 { namespace Ogre
{
   ...
   void ControllerUI::Avatar::ConditionChange(int Index)
   {
      ...
      if ((int)Conditions->getChildCount() > Index)
      {
         ...
         if (wnd->getChildCount() > 1)
         {
            ...
            {
            case StatNums::HITPOINTS:     wndBar->setTooltipText(UI_TOOLTIPTEXT_HPBAR);         break;
            case StatNums::MANA:          wndBar->setTooltipText(UI_TOOLTIPTEXT_MPBAR);         break;
            case StatNums::VIGOR:         wndBar->setTooltipText(UI_TOOLTIPTEXT_VIGBAR);        break;
            case StatNums::TOUGHERCHANCE: wndBar->setTooltipText(UI_TOOLTIPTEXT_TOUGHERCHANCE); break;
            }
            ...
            if (condition->Resource != nullptr && condition->Resource->Frames->Count > 0)
            {
               ...
               Util::CreateTextureA8R8G8B8(condition->Resource->Frames[0], oStrName, UI_RESGROUP_IMAGESETS, 0);
   ...
   bool UICallbacks::Avatar::OnBuffMouseClick(const CEGUI::EventArgs& e)
   {
      ...
      // single click requests object details
